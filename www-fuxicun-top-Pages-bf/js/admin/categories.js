...
(function() {
  ...
  /** 拖拽状态变量 */
  var dragSrcRow = null;
  /** 当前已加载的分类缓存（按 ID 索引），用于编辑弹窗查找完整对象 */
  var categoriesCache = {};
  ...
  async function loadCategories() {
    try {
      ...
      if (result.success) {
        // 缓存按 ID 索引，编辑时直接取（避免 onclick 拼接整个对象 → XSS）
        categoriesCache = {};
        (result.data || []).forEach(function(c) { categoriesCache[c.id] = c; });
  ...
  function renderCategories(categories) {
    ...
    tbody.innerHTML = categories.map(function(c) {
      return '<tr draggable="true" data-id="' + c.id + '" data-sort="' + c.sort_order + '">' +
        ...
        '<td class="actions">' +
          '<button onclick="showEditModal(' + c.id + ')">编辑</button>' +
