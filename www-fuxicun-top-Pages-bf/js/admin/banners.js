...
(function() {
  ...
  /** 拖拽状态变量 */
  var dragSrcRow = null;
  /** 当前已加载的轮播图缓存（按 ID 索引），编辑时从缓存取，避免 JSON.stringify 注入 onclick */
  var bannersCache = {};
  ...
  async function loadBanners() {
    try {
      ...
      if (result.success) {
        bannersCache = {};
        (result.data || []).forEach(function(b) { bannersCache[b.id] = b; });
  ...
  function renderBanners(banners) {
    ...
    tbody.innerHTML = banners.map(function(b, index) {
      return '<tr draggable="true" data-id="' + b.id + '" data-sort="' + b.sort_order + '">' +
        ...
        '<td class="actions">' +
          '<button onclick="showEditModal(' + b.id + ')">编辑</button>' +
  ...
  // ==============================
  // 编辑轮播图弹窗（接受 ID，从缓存取完整对象）
  // ==============================
  window.showEditModal = function(bannerId) {
    var banner = bannersCache[bannerId];
    if (!banner) {
      Toast.error('轮播图不存在，请刷新页面');
      return;
    }
    Modal.show({
      title: '编辑轮播图',
