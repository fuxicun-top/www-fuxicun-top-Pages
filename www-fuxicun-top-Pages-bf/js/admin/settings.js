...
(function() {
  ...
  async function loadConfig() {
    try {
      ...
      if (result.success) {
        ...
        // comment_review checkbox（comment_policy/like_policy 已被通用 select 循环填充）
        var cr = document.getElementById('comment_review');
        if (cr) {
          cr.checked = data.comment_review === 'true';
        }
      }
    } catch (e) {
      Toast.error('加载配置失败');
  ...
  async function saveConfig() {
    ...
    // checkbox → 字符串（site_config 是 TEXT）
    var crEl = document.getElementById('comment_review');
    data.comment_review = crEl && crEl.checked ? 'true' : 'false';
