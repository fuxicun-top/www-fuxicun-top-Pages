...
(function() {
  ...
  async function loadArticle() {
    ...
    try {
      ...
      // 评论策略：null 显示"继承全局"
      var policySel = document.getElementById('article-comment-policy');
      if (policySel) {
        policySel.value = article.comment_policy || '';
      }
  ...
  async function save(status) {
    ...
    // 评论策略（空字符串 → 继承全局，发送 null）
    var policySel = document.getElementById('article-comment-policy');
    if (policySel) {
      var policyVal = policySel.value;
      data.comment_policy = policyVal === '' ? null : policyVal;
    }
