...
async function getArticles(request, env) {
  ...
  // 构建缓存键（基于查询参数）
  const cacheKey = 'articles:list:' + page + ':' + pageSize + ':' + category + ':' + keyword;
  ...
  const articles = await dbQuery(
    env.FUXICUN_DB,
    'SELECT a.id, a.title, a.slug, a.excerpt, a.cover_image, a.views, a.likes, a.created_at, a.published_at, u.username as author_name, c.name as category_name, c.slug as category_slug FROM articles a LEFT JOIN users u ON a.author_id = u.id LEFT JOIN categories c ON a.category_id = c.id WHERE ' + where + ' ORDER BY a.published_at DESC LIMIT ? OFFSET ?',
...
async function createArticle(request, env) {
  ...
  try {
    const { title, content, excerpt, cover_image, category_id, comment_policy } = await request.json();
    ...
    // 仅 editor / admin 可设置 comment_policy（普通用户发的文章一律继承全局）
    const allowedPolicies = [null, 'open', 'login_required', 'closed'];
    let safeCommentPolicy = null;
    if (['editor', 'admin'].includes(auth.user.role) && comment_policy !== undefined) {
      if (!allowedPolicies.includes(comment_policy)) {
        return errorResponse('comment_policy 取值非法');
      }
      safeCommentPolicy = comment_policy;
    }
    ...
    const result = await dbRun(
      env.FUXICUN_DB,
      "INSERT INTO articles (title, slug, content, excerpt, cover_image, category_id, author_id, status, comment_policy, published_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [title, slug, content, excerpt || '', cover_image || '', category_id || null, auth.user.id, status, safeCommentPolicy, publishedAt]
    );
...
async function updateArticle(request, env, id) {
  ...
  try {
    ...
    // comment_policy：仅 editor/admin 可改；普通用户提交此字段会被忽略
    const allowedPolicies = [null, 'open', 'login_required', 'closed'];
    let nextCommentPolicy = article.comment_policy;
    if (['editor', 'admin'].includes(auth.user.role) && comment_policy !== undefined) {
      if (!allowedPolicies.includes(comment_policy)) {
        return errorResponse('comment_policy 取值非法');
      }
      nextCommentPolicy = comment_policy;
    }

    await dbRun(
      env.FUXICUN_DB,
      "UPDATE articles SET title = ?, content = ?, excerpt = ?, cover_image = ?, category_id = ?, comment_policy = ?, updated_at = datetime('now') WHERE id = ?",
      [title || article.title, content || article.content, excerpt ?? article.excerpt, cover_image ?? article.cover_image, category_id ?? article.category_id, nextCommentPolicy, id]
    );
...
async function likeArticle(request, env, id) {
  ...
  try {
    ...
    // 校验全局点赞策略（默认 open；admin 可改为 login_required 或 closed）
    let likePolicy = 'open';
    try {
      const cfg = await dbQueryFirst(env.FUXICUN_DB, "SELECT value FROM site_config WHERE key = 'like_policy'");
      if (cfg && cfg.value) likePolicy = cfg.value;
    } catch (e) { /* 取不到默认 open */ }

    if (likePolicy === 'closed') {
      return errorResponse('点赞功能已关闭', 403);
    }
    if (likePolicy === 'login_required' && !authUser) {
      return errorResponse('请先登录后再点赞', 401);
    }

    // 按身份决定按 user_id 或 guest_ip 去重
    let existing;
    if (authUser) {
      existing = await dbQueryFirst(
        env.FUXICUN_DB,
      ...
      if (guestIp === 'unknown') {
        return errorResponse('无法识别您的设备，请登录后点赞', 400);
      }
      existing = await dbQueryFirst(
        env.FUXICUN_DB,
        'SELECT id FROM likes WHERE article_id = ? AND guest_ip = ? AND user_id IS NULL',
        [id, guestIp]
      );
    }
    ...
    if (existing) {
      // 取消点赞
      if (authUser) {
        await dbRun(env.FUXICUN_DB, 'DELETE FROM likes WHERE article_id = ? AND user_id = ?', [id, authUser.id]);
      } else {
        await dbRun(env.FUXICUN_DB, 'DELETE FROM likes WHERE article_id = ? AND guest_ip = ? AND user_id IS NULL', [id, guestIp]);
      }
      ...
      // 点赞
      if (authUser) {
        await dbRun(env.FUXICUN_DB, 'INSERT INTO likes (article_id, user_id) VALUES (?, ?)', [id, authUser.id]);
      } else {
        await dbRun(env.FUXICUN_DB, 'INSERT INTO likes (article_id, guest_ip) VALUES (?, ?)', [id, guestIp]);
      }
