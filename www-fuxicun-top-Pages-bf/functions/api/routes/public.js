...
async function getPublicConfig(env) {
  ...
  const configs = await dbQuery(
    env.FUXICUN_DB,
    "SELECT key, value FROM site_config WHERE key IN ('site_name', 'site_description', 'site_keywords', 'contact_email', 'contact_phone', 'contact_address', 'icp_number', 'copyright_text', 'theme_primary_color', 'theme_primary_light', 'theme_primary_bg', 'theme_secondary_color', 'theme_memorial_dates', 'theme_memorial_mode')"
  );
  const result = {};
